...
const MODEL_CHAIN = [
  "gemini-2.5-flash",           // Primary stable endpoint for this API key
    ...
    if (farmer_id) {
      try {
        ...
        if (farmer) {
          ...
          // Fetch Activities
          const activities = await Activity.find({ farmer: farmer_id }).sort({ date: -1 }).limit(5).lean();
          ...
          const reminders = await Reminder.find({ farmer: farmer_id, is_completed: false }).sort({ due_date: 1 }).limit(10).lean();
